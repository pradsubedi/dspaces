...
extern int test_sub_run(int dims, 
	int* npdim, uint64_t *spdim, int timestep,
	size_t elem_size, int num_vars, int terminate, MPI_Comm gcomm);

void print_usage()
{
    fprintf(stderr, "Usage: test_sb <dims> np[0] .. np[dims-1] sp[0] ... sp[dims-1] <timesteps> [-s <elem_size>] [-c <var_count>] [-t]\n"
