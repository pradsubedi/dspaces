...
static bitmask_t bitTranspose(unsigned nDims, unsigned nBits,
                              bitmask_t inCoords)
...
typedef bitmask_t (*BitReader)(unsigned nDims, unsigned nBytes, char const *c,
                               unsigned y);
typedef void (*BitWriter)(unsigned d, unsigned nBytes, char *c, unsigned y,
                          int fold);
...
static bitmask_t getIntBits(unsigned nDims, unsigned nBytes, char const *c,
                            unsigned y)
{
...
#include <string.h>
static void propogateIntBits(unsigned d, unsigned nBytes, char *c, unsigned y,
                             int fold)
{
...
static void propogateIEEEBits(unsigned d, unsigned nBytes, char *cP, unsigned y,
                              int fold)
{
...
static void getIEEEinitValues(double const *c1, unsigned y, unsigned nDims,
                              unsigned *rotation, bitmask_t *bits,
                              bitmask_t *index)
{
...
static int hilbert_cmp_work(unsigned nDims, unsigned nBytes, unsigned nBits,
                            unsigned max, unsigned y, char const *c1,
                            char const *c2, unsigned rotation, bitmask_t bits,
                            bitmask_t index, BitReader getBits)
{
    bitmask_t const one = 1;
    bitmask_t const nd1Ones =
        ones(bitmask_t, nDims) >> 1; /* used in adjust_rotation macro */
    while(y-- > max) {
        ...
        if(diff) {
            ...
            return (((index ^ y ^ nBits) & 1) == (bits < (bits ^ diff))) ? -1
                                                                         : 1;
        }
        index ^= bits;
...
int hilbert_cmp(unsigned nDims, unsigned nBytes, unsigned nBits, void const *c1,
                void const *c2)
{
    bitmask_t const one = 1;
    bitmask_t bits = one << (nDims - 1);
    return hilbert_cmp_work(nDims, nBytes, nBits, 0, nBits, (char const *)c1,
                            (char const *)c2, 0, bits, bits, getIntBits);
}
...
int hilbert_ieee_cmp(unsigned nDims, double const *c1, double const *c2)
{
    ...
    return hilbert_cmp_work(nDims, 8, 64, max, max + 53, (char const *)c1,
                            (char const *)c2, rotation, bits, index,
                            getIEEEBits);
}
 ...
 *      nBits <= (sizeof bitmask_t) * (bits_per_byte)
 */
static unsigned hilbert_box_vtx_work(unsigned nDims, unsigned nBytes,
                                     unsigned nBits, int findMin, unsigned max,
                                     unsigned y, char *c1, char *c2,
                                     unsigned rotation, bitmask_t bits,
                                     bitmask_t index, BitReader getBits)
{
    bitmask_t const one = 1;
                         ...
                         int findMin, void *c1, void *c2)
{
    bitmask_t const one = 1;
    bitmask_t bits = one << (nDims - 1);
    return hilbert_box_vtx_work(nDims, nBytes, nBits, findMin, 0, nBits,
                                (char *)c1, (char *)c2, 0, bits, bits,
                                getIntBits);
}
                              ...
                              double *c2)
{
    ...
    getIEEEinitValues(c1, max + 53, nDims, &rotation, &bits, &index);
    return hilbert_box_vtx_work(nDims, 8, 64, findMin, max, max + 53,
                                (char *)c1, (char *)c2, rotation, bits, index,
                                getIEEEBits);
}
 ...
 *      nBits <= (sizeof bitmask_t) * (bits_per_byte)
 */
unsigned hilbert_box_pt_work(unsigned nDims, unsigned nBytes, unsigned nBits,
                        ...
                        int findMin, void *c1, void *c2)
{
    bitmask_t const one = 1;
    bitmask_t bits = one << (nDims - 1);
    return hilbert_box_pt_work(nDims, nBytes, nBits, findMin, 0, nBits,
                               (char *)c1, (char *)c2, 0, bits, bits,
                             ...
                             double *c2)
{
    ...
    if(c1Signs != c2Signs) {
        ...
        index = 1;
        hilbert_box_pt_work(nDims, 8, 64, findMin, IEEErepBits - 1, IEEErepBits,
                            (char *)c1, (char *)c2, rotation, bits, index,
    ...
    return hilbert_box_pt_work(nDims, 8, 64, findMin, max, max + 53, (char *)c1,
                               (char *)c2, rotation, bits, index, getIEEEBits,
                               propogateIEEEBits);
}
                      ...
                      int findPrev, void *c1V, void *c2V, void const *ptV)
{
    ...
    while(y-- > 0) {
         ...
         */
        {
             ...
             * this point
             */
            bitmask_t cornerdiff =
                (diff ^ reflection) ^ /* separate box crnrs */
