...
extern int test_get_run(int dims, 
	int* npdim, uint64_t *spdim, int timestep,
	size_t elem_size, int num_vars, int terminate, MPI_Comm gcomm);

void print_usage()
{
    fprintf(stderr, "Usage: test_reader <dims> np[0] .. np[dims-1] sp[0] ... sp[dims-1] <timesteps> [-s <elem_size>] [-c <var_count>] [-t]\n"
...
int main(int argc, char **argv)
{
	...
	if (parse_args(argc, argv, &dims, np, sp,
    		&timestep, &elem_size, &num_vars, &terminate) != 0) {
		goto err_out;
	}
	...
	ret = test_get_run(dims, np,
		sp, timestep, elem_size, num_vars, terminate, gcomm);
